...
merge m:m  conglome vivienda hogar ubigeo dominio estrato using "$dclea/mod100_`year'.dta", nogen 
merge m:m  conglome vivienda hogar ubigeo dominio estrato using "$tempo\pobreza_`year'.dta", nogen 
...
* Total de NBI (0 a 5)
gen nbi_total = nbi1 + nbi2 + nbi3 + nbi4 + nbi5
label var nbi_total "Número de NBI (0–5)"

* Clasificación INEI
gen pobre_nbi     = (nbi_total>=1)
gen pobre_ext_nbi = (nbi_total>=3)

label var pobre_nbi     "Pobre por NBI (>=1 NBI)"
label var pobre_ext_nbi "Pobre extremo por NBI (>=3 NBI)"

 
***Pobreza monetaria
rename pobre2 pob_monetaria
	label var pob_monetaria "Pobreza Monetaria"
	label define lbl_pob_monet 0 "No pobre monetario" 1 "Pobre monetario"
	label values pob_monetaria lbl_pob_monet
	
***Pobreza integrada – 4 categorías
gen pobre_integrado = .

replace pobre_integrado = 1 if pob_monetaria==0 & NBI1_POBRE==0   // Socialmente integrado
replace pobre_integrado = 2 if pob_monetaria==1 & NBI1_POBRE==0   // P. Coyuntural
replace pobre_integrado = 3 if pob_monetaria==0 & NBI1_POBRE==1   // P. Estructural
replace pobre_integrado = 4 if pob_monetaria==1 & NBI1_POBRE==1   // P. Crónico
...
gen pobre_integrada = (pob_monetaria==1 |pobre_nbi ==1)
...
fre nbi*
 keep conglome vivienda hogar codperso ubigeo dominio estrato facpob07 year sex age g_edad ecivil p207 year2 educ sch analfa lmatr factor07 seguro dic_segur limitacion croni morbilid dni nconglome ocu500 ocupinf fac500a lw lwage sec_ocu sector_econ funcion nempljh act_empr race nbi1 nbi2 nbi3 nbi4 nbi5 ag_pot enerlectr internet prop_viv tit_prop sunarp urban zone mes percepho mieperho totmieho ingmo1hd ingmo2hd inghog1d inghog2d gashog1d gashog2d ld estrsocial linpe linea pobreza anio facpob NBI1_POBRE NBI2_POBRE gpcm pobre3 pob_monetaria year_2 factor_p fac500_p sub_conglome lineav_rpl lineav pobrezav depto_  pobre_integrado pob_mon_puno pob_mont_100 pob_mont_puno_100 pob_intg_100 pob_intg_puno_100 desag numinthog ingperc l_ingre   pobre_nbi pobre_ext_nbi  integ_social pob_coyuntural pob_estructural pob_cronico panel
